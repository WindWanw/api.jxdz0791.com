...
<script>
    ...
    function highlight() {
        let standardChildren = standard.children[0].children[0];
        var jia = document.getElementsByClassName("jia")[0];
        if (jia) jia.click();
        let children = standardChildren.children[1];
        clickParent(children);
        children.children[0].children[1].children[0].click();
        setTimeout(() => {
            setClassName(
                children.children[0].children[1].children[0].children[0], ["active"]);
        });
    }
